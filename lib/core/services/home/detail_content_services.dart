...
class DetailContentService {
  Future<Publication> fetchContentDetail(String contentId) async {
    final token = await StorageUtils.getToken();

    final response = await http.get(
      Uri.parse(ApiUrls.contentDetail(contentId)),
      headers: {
    ...
    if (response.statusCode == 200) {
      ...
      return Publication.fromJson(data['data']); // pastikan key benar
    } else {
      throw Exception("Gagal mengambil detail konten: ${response.body}");
    }
  }
